...
describe('update', () => {
  ...
  it('should get tableList in right action with subquery', () => {
    const sql = `UPDATE empdb.employees t1 INNER JOIN (select t2.percentage,t2.performance from empdb.merits t2) as t3 ON t1.performance = t3.performance SET t1.salary = t1.salary + t1.salary * t3.percentage;`
  ...
  it('should get tableList for DELETE with JOIN', () => {
    const sql = `DELETE t1 FROM t1 INNER JOIN  t2 ON t2.ref = t1.id;`
    const ast = parser.parse(sql)
